...
export class SearchResults {
  ...
  datasource = new kendo.data.DataSource({
    transport: {
      read: (options) => {
        ...
        this.loadData()
          .then((daily) => {
            console.log(' daily datasource ', daily.length);// inv[0]);
            options.success(daily);
          });
      },
  ...
  loadData() {
    ...
    if (this.appService.searchDataLoaded) {
      ...
      // return  Promise.all([
      //   this.dataService.loadSearchDaily(this.queryParams)
      // ]).then(values => {
      //   this.origItems = values[0];
      //   daily = this.origItems;
...
this.origItems = {};
  ...
  details(e) {
    ...
    let selectedRow = grid.select();
    let dataItem = grid.dataItem(selectedRow);
     let rt2 = '#/daily/data/' + dataItem.DAILY_ID

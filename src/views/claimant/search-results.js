...
export class SearchResults {
  ...
  loadData() {
    ...
    if (this.appService.searchDataLoaded) {
      ...
      return  Promise.all([
        this.dataService.loadSearchClaimant(this.queryParams)
      ]).then(values => {
        this.origItems = values[0];
        claimant = this.origItems;
        console.log('claimant ', claimant.length)
        return claimant
  ...
  details(e) {
    ...
    let selectedRow = grid.select();
    let dataItem = grid.dataItem(selectedRow);
    let rt2 = '#/claimant/data/' + dataItem.CLAIMANT_ID;
